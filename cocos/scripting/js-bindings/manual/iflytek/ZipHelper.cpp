...
public:
	static bool unzipFile(const string& inputFilePath, const string& outFileDirPath, bool overWrite, const string& password = "")
	{
		...
		bool createDirResult = FileUtils::getInstance()->createDirectory(outFileDirPath); // 创建输出目录
		if (!createDirResult)
		{
			return false;
		}
		...
		if (ret == UNZ_OK) {
			...
			do {
				...
				std::string fullPath = outFileDirPath + StringUtils::format("/%s", filename);
	...
	if (argc >= 2)
	{
		if (argv[0].isString() && argv[1].isString())
		{
			...
			if (argc == 3)
			{
				if (argv[2].isBoolean())
				{
					isOverWrite = argv[2].toBoolean();
				}
				else
				{
					JS_ReportError(cx, "wrong arg 3, must be Boolean type");
					return false;
				}
			}
			...
			if (argc == 4)
			{
				if (argv[3].isString())
				{
					jsval_to_std_string(cx, argv[3], &password);
				}
				else
				{
					JS_ReportError(cx, "wrong arg 4, must be String type");
					return false;
				}
			}

			bool result = cobj->unzipFile(inputFile, outputFileDir, isOverWrite, password);
...
void register_jsb_zip_helper(JSContext *cx, JS::HandleObject global)
{
	...
	static JSFunctionSpec funcs[] = {
		JS_FN("unzipFile", js_cocos2dx_extension_ZipHelper_unzipFile, 4, JSPROP_PERMANENT | JSPROP_ENUMERATE),
		JS_FS_END
	};
